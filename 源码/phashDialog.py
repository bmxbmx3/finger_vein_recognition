...
class phashDialog(wx.Dialog):

    def __init__(self, parent):
        wx.Dialog.__init__(self, parent, id=wx.ID_ANY, title=u"感知哈希", pos=wx.DefaultPosition, size=wx.Size(-1, -1),
        ...
        self.m_staticText19 = wx.StaticText(self.pan, wx.ID_ANY, u"重设哈希尺度：", wx.DefaultPosition, wx.DefaultSize, 0)
        self.m_staticText19.Wrap(-1)
        gSizer12.Add(self.m_staticText19, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALIGN_RIGHT, 5)

        self.highfreq_factor = wx.SpinCtrl(self.pan, wx.ID_ANY, wx.EmptyString, wx.DefaultPosition, wx.Size(80, -1),
                                           wx.SP_ARROW_KEYS, 1, 30, 4)
        gSizer12.Add(self.highfreq_factor, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALIGN_LEFT, 5)
    ...
    def OnOk(self, event):
        phashData = {"hash_size": self.hash_size.GetValue(), "highfreq_factor": self.highfreq_factor.GetValue(),
                     "THRESHOLD": self.THRESHOLD.GetValue()}
        with builtins.open("phashData.json", "w", encoding="UTF-8") as fp:
            json.dump(phashData, fp)
        pub.sendMessage("phashData", phashData=phashData)
        self.Destroy()
        self.Parent.Enable(True)
        self.Parent.SetFocus()
        event.Skip()
