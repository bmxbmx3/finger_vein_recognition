...
class ahashDialog(wx.Dialog):

    def __init__(self, parent):
        wx.Dialog.__init__(self, parent, id=wx.ID_ANY, title=u"平均散列", pos=wx.DefaultPosition, size=wx.Size(-1, -1),
    ...
    def OnOk(self, event):
        ahashData = {"hash_size": self.hash_size.GetValue(), "THRESHOLD": self.THRESHOLD.GetValue()}
        with builtins.open("ahashData.json", "w", encoding="UTF-8") as fp:
            json.dump(ahashData, fp)
        pub.sendMessage("ahashData", ahashData=ahashData)
        self.Destroy()
        self.Parent.Enable(True)
        self.Parent.SetFocus()
        event.Skip()
